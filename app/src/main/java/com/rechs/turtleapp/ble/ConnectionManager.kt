...
import com.rechs.turtleapp.MainActivity
import com.rechs.turtleapp.TurtleListeners
...
object ConnectionManager {
    ...
    fun readCharacteristic(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) : Int {
        if (device.isConnected() && characteristic.isReadable()) {
            enqueueOperation(CharacteristicRead(device, characteristic.uuid))
            if(characteristic.value == null) return 0;
            return TurtleListeners.Companion.convertByteArrayToInt(characteristic.value);
